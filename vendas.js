...
const VD = (function(){
  ...
  function renderKpis(k,kc){ const box=f('kpis'); if(!k){box.innerHTML='';return;} const cards=[['Faturamento bruto',brl(k.faturamento_bruto)],['Pedidos',Number(k.qtd_pedidos||0).toLocaleString('pt-BR')],['Ticket médio',brl(k.ticket_medio)],['Comissão total',brl(k.total_comissao)],['Receita comercial',brl(k.receita_comercial)]]; if(kc) cards.push(['Faltam conferir',Number(kc.faltam||0).toLocaleString('pt-BR')]); box.innerHTML=cards.map(c=>`<div class="kpi"><div class="lbl">${c[0]}</div><div class="val">${c[1]}</div></div>`).join(''); }
  ...
  function renderTabela(){
    ...
    tb.innerHTML=LINHAS.map(l=>`<tr class="${l.conferido?'':'pendente'}" onclick="VD.abrir(${l.id})">
      <td>${dataBr(l.data_compra)}</td><td>${l.canal||'—'}</td><td>${l.tipo_envio||'—'}</td><td>${l.id_pedido||'—'}</td><td>${l.modelo||'—'}</td>
  ...
  async function conf(id,valor,elem){ elem.disabled=true; try{ await rpc('cn_marcar_conferido',{p_usuario_id:USER.id,p_venda_id:id,p_conferido:valor}); const l=LINHAS.find(x=>x.id===id); if(l)l.conferido=valor; const tr=elem.closest('tr'); tr.classList.toggle('pendente',!valor); tr.querySelector('.conf-lbl').textContent=valor?'Conferido':'Pendente'; }catch(e){ elem.checked=!valor; alert('Não foi possível marcar: '+(e.message||e)); } finally{ elem.disabled=false; } }
  ...
  function abrir(id){ if(!temPermissao('vendas.editar')) return; const l=LINHAS.find(x=>x.id===id); if(!l)return; EDIT_ID=id; f('drawer-erro').textContent='';
    f('e-data').value=l.data_compra||''; f('e-canal').value=l.canal||''; f('e-tenvio').value=l.tipo_envio||''; f('e-uf').value=l.uf||''; f('e-cliente').value=l.cliente||''; f('e-modelo').value=l.modelo||'';
  ...
  function prev(){ const q=parseFloat(f('e-qtd').value)||0,vu=parseFloat(f('e-vunit').value)||0,co=parseFloat(f('e-comissao').value)||0,fr=parseFloat(f('e-frete').value)||0,fe=parseFloat(f('e-fextra').value)||0; const t=vu*q,nf=t+fe,rc=nf-co-fr-fe,lu=nf>0?rc/nf*100:0; f('c-total').value=brl(t); f('c-nf').value=brl(nf); f('c-receber').value=brl(rc); f('c-lucro').value=nf>0?lu.toFixed(2)+'%':'—'; }
