...
const EN = (function(){
  ...
  function renderKpis(k){ 
    ...
    const cards=[
      ['Total de envios',Number(k.total||0).toLocaleString('pt-BR')],
      ['Faltam conferir',Number(k.faltam||0).toLocaleString('pt-BR')],
      ['Custo Total com Frete',brl(k.soma_transporte)],
      ['Saldo de Frete',brl(k.soma_diferenca)],
      ['Total Frete Extra',brl(k.soma_frete_extra)],
      ['Total de envios entregues',Number(k.qtd_entregues||0).toLocaleString('pt-BR')],
      ['Total de envios a caminho',Number(k.qtd_a_caminho||0).toLocaleString('pt-BR')],
      ['Total de envios atrasados',Number(k.qtd_atrasados||0).toLocaleString('pt-BR')],
      ['Total de envios em processo de devolução',Number(k.qtd_devolucao||0).toLocaleString('pt-BR')],
      ['Total de envios aguardando transportadora',Number(k.qtd_aguardando_transp||0).toLocaleString('pt-BR')]
    ]; 
...
'); const blob=new Blob(['﻿'+csv],{type:'text/csv;charset=utf-8'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='envios_carrinhos_net.csv'; a.click(); }catch(e){ alert('Erro ao exportar: '+(e.message||e)); } finally{ b.disabled=false; b.textContent=t; } }
  ...
  function bind(){
    // Removemos os events onchange dos selects para que apenas o Botão atualize
