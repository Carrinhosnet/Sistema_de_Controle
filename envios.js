...
const EN = (function(){
  ...
  function filtros(){ return { p_usuario_id:USER.id, p_mes:f('mes').value||null, p_canal:f('canal').value||null, p_status:f('status').value||null, p_transportadora:f('transp').value||null, p_conferido:f('conf').value===''?null:(f('conf').value==='true'), p_editado:f('edit').value===''?null:(f('edit').value==='true'), p_busca:f('busca').value.trim()||null }; }
  ...
  function renderKpis(k){ const box=f('kpis'); if(!k){box.innerHTML='';return;} const cards=[['Total de envios',Number(k.total||0).toLocaleString('pt-BR')],['Faltam conferir',Number(k.faltam||0).toLocaleString('pt-BR')],['Editados',Number(k.editados||0).toLocaleString('pt-BR')],['Total transporte',brl(k.soma_transporte)],['Diferença total',brl(k.soma_diferenca)]]; box.innerHTML=cards.map(c=>`<div class="kpi"><div class="lbl">${c[0]}</div><div class="val">${c[1]}</div></div>`).join(''); }
  ...
  function renderTabela(){ const tb=f('tbody'); if(!LINHAS.length){ tb.innerHTML='<tr><td colspan="20" class="empty">Nenhum envio encontrado.</td></tr>'; return; } const podeConf=temPermissao('envios.conferir');
    tb.innerHTML=LINHAS.map(l=>{ const dif=Number(l.diferenca||0); return `<tr class="${l.conferido?'':'pendente'}" onclick="EN.abrir(${l.id})">
      <td>${l.editado?'<span title="Editado" style="color:var(--warn)">✎ </span>':''}${dataBr(l.data_compra)}</td><td>${mesLabel(l.mes)||'—'}</td><td>${l.canal||'—'}</td><td>${l.id_pedido||'—'}</td><td>${l.modelo||'—'}</td><td class="num">${l.quantidade??'—'}</td><td>${l.cliente||'—'}</td><td>${l.uf||'—'}</td>
